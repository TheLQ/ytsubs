...
export function parseSubscriptionsOpml(
    ...
    const body = filterChildrenFormatting(opml.children)[0] as parseXml.XmlElement;
    if (body.name != "body") {
    ...
    ) as parseXml.XmlElement[]).map(outline => {
...
export function parseChannelFeed(xmlContent: string): VideoStorage[] {
    try {
      const xml = parseXml(xmlContent);
      const feed = filterChildrenFormatting(xml.children)[0] as parseXml.XmlElement;
      if (feed.name != "feed") {
